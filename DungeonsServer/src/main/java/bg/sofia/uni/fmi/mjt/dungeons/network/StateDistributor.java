...
public class StateDistributor {
    ...
    public StateDistributor(PlayerManager playerManager, ActorRepository actorRepository) {
    ...
    public void distributeState() {
        for (Player player : playerManager.getAllPlayers()) {
            byte[] playerPackageBytes = serializePlayerSegment(player.id());
    ...
    private byte[] serializePlayerSegment(int playerId) {
        PlayerSegment playerSegment = new PlayerSegment(playerId, actorRepository);
